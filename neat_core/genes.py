...
A trimmed, self-contained NEAT implementation scoped to a single car's brain -
a handful of sensors in, steering and throttle out. No reserved capacity, no
open-ended trait system: this project trains a fixed task for a fixed number
of generations, so the genome only needs to be exactly as flexible as that.
...
class Innovations:
    """Global historical markings, shared by the whole population. Two genomes
    that grow the same structure independently must get the same innovation
    number, or crossover cannot line them up."""
...
class Genome:
    ...
    @classmethod
    def minimal(cls, n_in, n_out, innov, cfg):
        """Inputs + bias + outputs, fully connected, no hidden layer."""
        g = cls()
    ...
    def copy(self):
        return Genome(dict(self.nodes), {i: replace(c) for i, c in self.conns.items()})
